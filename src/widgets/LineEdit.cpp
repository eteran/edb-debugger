...
LineEdit::LineEdit(QWidget *parent) : QLineEdit(parent), clear_button_(new QToolButton(this)) {
	...
	connect(clear_button_, &QToolButton::clicked, this, &LineEdit::clear);
	...
	setMinimumSize(std::max(msz.width(), clear_button_->sizeHint().height() + frameWidth * 2 + 2), std::max(msz.height(), clear_button_->sizeHint().height() + frameWidth * 2 + 2));
}
