...
void ArchProcessor::setup_register_view(RegisterListWidget *category_list) {
	...
	if(edb::v1::debugger_core) {
		...
		if(QTreeWidgetItem *const fpu = category_list->addCategory(tr("FPU"))) {
			register_view_items_[0x18] = create_register_item(fpu, "R0");
			register_view_items_[0x19] = create_register_item(fpu, "R1");
			register_view_items_[0x1a] = create_register_item(fpu, "R2");
			register_view_items_[0x1b] = create_register_item(fpu, "R3");
			register_view_items_[0x1c] = create_register_item(fpu, "R4");
			register_view_items_[0x1d] = create_register_item(fpu, "R5");
			register_view_items_[0x1e] = create_register_item(fpu, "R6");
			register_view_items_[0x1f] = create_register_item(fpu, "R7");
		}
...
void ArchProcessor::update_register_view(const QString &default_region_name, const State &state) {
	...
	for(int i = 0; i < 8; ++i) {
		...
		register_view_items_[24 + i]->setText(0, QString("%1R%2: %3 0x%4%5%6").arg(top==i?"=>":"  ").arg(i).arg(tag.leftJustified(8)).arg(current.toHexString()).arg(valueString).arg(typeString));
