...
namespace {
	...
	struct elf32_model {
		...
		typedef Elf32_Shdr Elf_section_header;
		typedef Elf32_Sym  Elf_symbol;
		typedef Elf32_Rela Elf_relocationA;
		typedef Elf32_Rel  Elf_relocation;
	...
	struct elf64_model {
		...
		typedef Elf64_Shdr Elf_section_header;
		typedef Elf64_Sym  Elf_symbol;
		typedef Elf64_Rela Elf_relocationA;
		typedef Elf64_Rel  Elf_relocation;
	...
	*/
	...
	void process_symbols(const void *p, size_t size) {
		...
		typedef typename M::Elf_header         Elf_header;
		typedef typename M::Elf_section_header Elf_section_header;
		typedef typename M::Elf_symbol         Elf_symbol;
		typedef typename M::Elf_relocationA    Elf_relocationA;
		typedef typename M::Elf_relocation     Elf_relocation;
		typedef typename M::symbol             symbol;

		const uintptr_t base = reinterpret_cast<uintptr_t>(p);

		const Elf_header *const header                 = static_cast<const Elf_header*>(p);
		const Elf_section_header *const sections_begin = reinterpret_cast<Elf_section_header*>(base + header->e_shoff);
		const Elf_section_header *const sections_end   = sections_begin + header->e_shnum;
		...
		for(const Elf_section_header *section = sections_begin; section != sections_end; ++section) {
		...
		for(const Elf_section_header *section = sections_begin; section != sections_end; ++section) {
			...
			case SHT_RELA:
				{
					int n = 0;
					const Elf_relocationA *relocation = reinterpret_cast<Elf_relocationA*>(base + section->sh_offset);

					for(size_t i = 0; i < section->sh_size / section->sh_entsize; ++i) {

						const int sym_index              = M::elf_r_sym(relocation[i].r_info);
						const Elf_section_header *linked = &sections_begin[section->sh_link];
						const Elf_symbol *symbol_tab     = reinterpret_cast<Elf_symbol*>(base + linked->sh_offset);
			...
			case SHT_REL:
				{
					int n = 0;
					const Elf_relocation *relocation = reinterpret_cast<Elf_relocation*>(base + section->sh_offset);
		...
		for(const Elf_section_header *section = sections_begin; section != sections_end; ++section) {
			...
			case SHT_DYNSYM:
				{
					Elf_symbol *symbol_tab = reinterpret_cast<Elf_symbol*>(base + section->sh_offset);
					const char *string_tab = reinterpret_cast<const char*>(base + sections_begin[section->sh_link].sh_offset);

					for(size_t i = 0; i < section->sh_size / section->sh_entsize; ++i) {

						const Elf_section_header *related_section = 0;
