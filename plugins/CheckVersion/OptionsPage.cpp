...
void OptionsPage::showEvent(QShowEvent *event) {
	...
	QSettings settings;
	ui.checkBox->setChecked(settings.value(QStringLiteral("CheckVersion/check_on_start.enabled"), true).toBool());
}
...
void OptionsPage::on_checkBox_toggled(bool checked) {
	...
	QSettings settings;
	settings.setValue(QStringLiteral("CheckVersion/check_on_start.enabled"), ui.checkBox->isChecked());
}
